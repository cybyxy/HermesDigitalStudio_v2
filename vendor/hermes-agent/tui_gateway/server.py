...
def _history_to_messages(history: list[dict]) -> list[dict]:
    ...
    for m in history:
        ...
        if not (m.get("content") or "").strip():
            continue
        msg = {"role": role, "text": m.get("content") or ""}
        if m.get("reasoning"):
...
def _(rid, params: dict) -> dict:
    sid = params.get("session_id") or uuid.uuid4().hex[:8]
    key = params.get("session_key") or _new_session_key()
...
@method("session.history_by_key")
def _(rid, params: dict) -> dict:
    """从 state.db 直接读取指定 session_key 的对话历史。

    用于 Studio 后端重启后，通过持久化的 session_key 恢复会话历史，
    不依赖内存中的 session 状态。
    """
    session_key = params.get("session_key")
    if not session_key:
        return _err(rid, -1, "session_key 不能为空")
    db = _get_db()
    if db is None:
        return _err(rid, -2, "state.db 不可用")
    history = []
    try:
        history = db.get_messages_as_conversation(
        ...
        return _err(rid, -3, f"无法读取历史: {exc}")
    return _ok(
        rid,
        {
            "count": len(history),
